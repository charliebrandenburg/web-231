...
document.getElementById("cashBox").addEventListener("change", runTheRegister);
document.getElementById("billBox").addEventListener("change", runTheRegister);
...
function runTheRegister() {
   ...
   return changeBox.value = formatCurrency(changeValue); // format the change as currency

   return calcChange(changeValue); // Determine the units of currency needed for the change
}
...
// Try statement
  try {
    if (!(bill20Amt >= 1 && bill20Amt <= 20))
      throw "Cash amount doesn't cover the bill";
    testFormCompleteness();
   } catch(error) {
    messageElement.innerHTML = error;
    messageHeadElement.innerHTML = "warning";
   }
